...
resource "aws_lambda_function" "api" {
  ...
  function_name    = "serverless-user-api"
...
resource "aws_api_gateway_rest_api" "api" {
  ...
  tags = {
    Name        = "Serverless User API Gateway"
    Environment = var.environment
    Project     = var.project_name
  }
}
...
resource "aws_api_gateway_deployment" "api" {
  ...
  # Trigger new deployment when Lambda code changes
  triggers = {
...
# API Gateway Stage (modern way to manage stages)
...
resource "aws_s3_bucket" "frontend" {
  bucket        = "serverless-frontend-${random_string.bucket_suffix.result}"
  force_destroy = true
